...
import { ArrowLeft, FileSignature, CheckCircle, XCircle, BadgePercent } from 'lucide-react';
...
export default function AdjustmentPage() {
  ...
  const asset = getAssetDetails(id, assetType);
  ...
  const negotiatedValue = 'price' in asset ? ('quantity' in asset ? asset.price * asset.quantity : asset.price) : 50000;
  ...
  const finalContractText = contractTemplate
    ...
    .replace('[VALOR_ATIVO]', new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format('amount' in asset ? asset.amount : negotiatedValue))
