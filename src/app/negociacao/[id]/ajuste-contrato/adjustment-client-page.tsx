...
export function AdjustmentClientPage({ assetId, assetType, asset }: { assetId: string, assetType: AssetType, asset: Asset | null }) {
  ...
  const handleAccept = async () => {
    ...
    try {
        ...
        // Se o contrato foi congelado, envie o e-mail de validação
        if (acceptResponse.contract?.step === 2) {
             toast({ title: "Contrato Congelado!", description: "Ambas as partes aceitaram. Enviando e-mail de validação..." });
  ...
  const finalContractText = React.useMemo(() => {
      ...
      const parties = {
        seller: { 
            name: contract.fields.seller.razaoSocial || '[Razão Social do Vendedor]', 
            doc: contract.fields.seller.cnpj || '[CNPJ do Vendedor]', 
            address: contract.fields.seller.endereco || '[Endereço do Vendedor]',
            ie: contract.fields.seller.ie || '[IE do Vendedor]',
            repName: 'N/A', repDoc: 'N/A', repRole: 'N/A'
        },
        buyer: { 
            name: contract.fields.buyer.razaoSocial || '[Razão Social do Comprador]', 
            doc: contract.fields.buyer.cnpj || '[CNPJ do Comprador]', 
            address: contract.fields.buyer.endereco || '[Endereço do Comprador]',
            ie: contract.fields.buyer.ie || '[IE do Comprador]',
        }
      }
            ...
            <CardContent className="space-y-6">
                <div className={cn("p-4 border rounded-lg", isSeller ? 'bg-background' : 'bg-muted/40')}>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2"><Label>Razão Social</Label><Input value={fields.seller.razaoSocial} onBlur={(e) => handleFieldChange('seller', 'razaoSocial', e.target.value)} defaultValue={fields.seller.razaoSocial} disabled={!isSeller || isFrozen} /></div>
                        <div className="space-y-2"><Label>CNPJ</Label><Input value={fields.seller.cnpj} onBlur={(e) => handleFieldChange('seller', 'cnpj', e.target.value)} defaultValue={fields.seller.cnpj} disabled={!isSeller || isFrozen} /></div>
                        <div className="space-y-2"><Label>Inscrição Estadual</Label><Input value={fields.seller.ie} onBlur={(e) => handleFieldChange('seller', 'ie', e.target.value)} defaultValue={fields.seller.ie} disabled={!isSeller || isFrozen} /></div>
                        <div className="space-y-2"><Label>Endereço Completo</Label><Input value={fields.seller.endereco} onBlur={(e) => handleFieldChange('seller', 'endereco', e.target.value)} defaultValue={fields.seller.endereco} disabled={!isSeller || isFrozen} /></div>
                     </div>
                </div>
                 <div className={cn("p-4 border rounded-lg", isBuyer ? 'bg-background' : 'bg-muted/40')}>
                    <h4 className="font-semibold mb-2">Campos do Comprador</h4>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2"><Label>Razão Social</Label><Input value={fields.buyer.razaoSocial} onBlur={(e) => handleFieldChange('buyer', 'razaoSocial', e.target.value)} defaultValue={fields.buyer.razaoSocial} disabled={!isBuyer || isFrozen} /></div>
                        <div className="space-y-2"><Label>CNPJ</Label><Input value={fields.buyer.cnpj} onBlur={(e) => handleFieldChange('buyer', 'cnpj', e.target.value)} defaultValue={fields.buyer.cnpj} disabled={!isBuyer || isFrozen} /></div>
                        <div className="space-y-2"><Label>Inscrição Estadual</Label><Input value={fields.buyer.ie} onBlur={(e) => handleFieldChange('buyer', 'ie', e.target.value)} defaultValue={fields.buyer.ie} disabled={!isBuyer || isFrozen} /></div>
                        <div className="space-y-2"><Label>Endereço Completo</Label><Input value={fields.buyer.endereco} onBlur={(e) => handleFieldChange('buyer', 'endereco', e.target.value)} defaultValue={fields.buyer.endereco} disabled={!isBuyer || isFrozen} /></div>
                    </div>
                </div>
            </CardContent>
            {!isFrozen && (
