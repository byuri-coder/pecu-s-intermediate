...
import { z } from 'zod';
import { Loader2, Send, User, Sparkles, FileText, DollarSign, Bookmark, Landmark } from 'lucide-react';
...
type ChatMode = 'economic' | 'tax';

const modeConfig = {
  economic: {
    welcome: {
      title: 'Como posso ajudar com seus cálculos econômicos?',
      example: 'Ex: "Qual o montante de juros compostos para um principal de R$1000 a 1% ao mês por 12 meses?"'
    },
    inputPlaceholder: 'Digite sua pergunta sobre juros, VPL, TIR, etc...',
  },
  tax: {
    welcome: {
      title: 'Como posso ajudar com suas questões tributárias?',
      example: 'Ex: "Como calcular o saldo credor de ICMS?"'
    },
    inputPlaceholder: 'Digite sua pergunta sobre ICMS, PIS/COFINS, etc...',
  },
};

export function AgentChat() {
  const [chatMode, setChatMode] = React.useState<ChatMode>('economic');
  const [conversations, setConversations] = React.useState<{ [key in ChatMode]: Message[] }>({
    economic: [],
    tax: [],
  });
  ...
  const onSubmit: SubmitHandler<ChatFormValues> = (data) => {
    ...
    startTransition(async () => {
      ...
      if (result.answer) {
        ...
        setConversations((prev) => ({ ...prev, [chatMode]: [...prev[chatMode], assistantMessage] }));
      } else {
        const errorMessage: Message = {
          role: 'assistant',
          content: 'Desculpe, não consegui processar sua solicitação. Tente novamente.',
        };
        setConversations((prev) => ({ ...prev, [chatMode]: [...prev[chatMode], errorMessage] }));
      }
    });
  };
          ...
          <div className="space-y-6">
            {messages.length === 0 && (
              <div className="text-center text-muted-foreground pt-10">
                ...
                <p className="mt-4 font-semibold">{modeConfig[chatMode].welcome.title}</p>
                <p className="text-xs">{modeConfig[chatMode].welcome.example}</p>
              </div>
            )}
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex items-center gap-1">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    type="button"
                    variant={chatMode === 'economic' ? 'secondary' : 'ghost'}
                    size="icon"
                    onClick={() => setChatMode('economic')}
                    disabled={isPending}
                  >
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    type="button"
                    variant={chatMode === 'tax' ? 'secondary' : 'ghost'}
                    size="icon"
                    onClick={() => setChatMode('tax')}
                    disabled={isPending}
                  >
                ...
                render={({ field }) => (
                  <FormItem className="flex-1">
                    <FormControl>
                      <Input
                        {...field}
                        placeholder={modeConfig[chatMode].inputPlaceholder}
                        autoComplete="off"
                        disabled={isPending}
